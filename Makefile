...
clean: 
	...
	rm -f $(SAM_OUTPUT)
deploy:
	aws cloudformation deploy --template-file $(SAM_OUTPUT) --stack-name $(STACK_NAME) --capabilities CAPABILITY_IAM --parameter-overrides Environment=$(ENV) QueueName=$(QueueName) RouteURL=$(RouteURL)
